...
vec3_t			g_vright = {1, 1, 1};		// needs to be set to viewer's right in order for chrome to work
float			g_lambert = 1.5;
...
void StudioModel::SetUpBones ( void )
{
	...
	if (pseqdesc->numblends > 1 /* && !m_framepaused */)
	{
		float				s;
...
void StudioModel::ModifyPause(void)
{
	m_framepaused ? m_framepaused = false : m_framepaused = true;
	// printf("paused: %i\n", m_framepaused);
}

bool StudioModel::IsFramePaused(void)
{
	return m_framepaused;
}

int StudioModel::SetFrame(int iValue)
{
	mstudioseqdesc_t* pseqdesc = (mstudioseqdesc_t*)((byte*)m_pstudiohdr + m_pstudiohdr->seqindex) + m_sequence;

	ModifyPause();

	if (iValue > pseqdesc->numframes)
		m_frame = 0;
	if (iValue < 0)
...
bool StudioModel::IsFiltered(void)
{
	return m_filtered;
}

void StudioModel::ModifyFilter(void)
{
	m_filtered ? m_filtered = false : m_filtered = true;
	DrawModel();
}
